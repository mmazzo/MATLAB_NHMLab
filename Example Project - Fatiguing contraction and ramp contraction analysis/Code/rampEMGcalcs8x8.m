function dat = rampEMGcalcs8x8(dat)
dat = dat;
        ...
        for r=1:size(Ind,1)
            for c=1:size(Ind,2)
                ...
                for r = 1:8 % for each row
                    ...
                    dat.(time).(contr).diffEMG.bycolumn{r,4} = dat.(time).(contr).SIG_filt{r,4} - dat.(time).(contr).SIG_filt{r,5};
                    dat.(time).(contr).diffEMG.bycolumn{r,5} = dat.(time).(contr).SIG_filt{r,5} - dat.(time).(contr).SIG_filt{r,6};
                    dat.(time).(contr).diffEMG.bycolumn{r,6} = dat.(time).(contr).SIG_filt{r,6} - dat.(time).(contr).SIG_filt{r,7};
                    dat.(time).(contr).diffEMG.bycolumn{r,7} = dat.(time).(contr).SIG_filt{r,7} - dat.(time).(contr).SIG_filt{r,8};
                    % RMS
                    for c = 1:7
